...
//
//    val pairs = for (x <- occurrences;
//                     y <- (1 to x._2)) yield (x._1, y)
//    ((0 to occurrences.size).flatMap(pairs.combinations(_)) filter( filterDup(_))).toList
//
//  }
//
//
...
//  }
  ...
  // Recursion if list is empty return empty list else
  // 1) build combination of tail
  // 2) concat tail combination with elem combinations
  // 3) elem combination is generated by genElemCombination
  //      loop through char occurrence and generate new elem and connect with each combination in combList
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match {
      case List() => List(List())
      case xh :: xt => {
        val tailCombinations = combinations(xt)
        tailCombinations ++ genElemCombination(xh,tailCombinations)
      }
    }

  def genElemCombination(elem: (Char, Int) ,combList: List[Occurrences]): List[Occurrences] = {
    for (occ <- combList;
          i <- (1 to elem._2)) yield (elem._1,i) :: occ
  }
